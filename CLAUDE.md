...
**Group liquidity — `Cash UK & UAE` sheet (added in the 26-Jun-2026 report):** Finance now ships two extra sheets, `UK & UAE` and `Cash UK & UAE`. The latter is the clean executive consolidation parsed by `extract_group_balances(wb)` (col B=label, C=balance in local ccy, D=exch. rate, **E=Amount ₦, F=Encumbered ₦, G=Available ₦**). It carries a row per entity/account (labels start with `Seamfix …`, e.g. `Seamfix Nigeria — NGN/USD`, `Seamfix UK — GBP/EUR`, `Seamfix UAE — USD/AED/EUR`), investment-balance rows, an `AIF Investment` row, and a `GRAND TOTAL — All Entities & Portfolios` row holding the gross/encumbered/available headline. The parser returns `{entities[], gross_ngn, encumbered_ngn, available_ngn, aif_ngn, aif_usd, uk_uae_ngn}` or `None` when absent (older reports → Nigeria-only view, unchanged). **`uk_uae_ngn` is the sum of the UK & UAE entity rows ONLY** — Nigeria's NGN+USD cash is already counted by the legacy `Cash Report` tab (in `total_cash_ngn`), so adding the whole grand total would double-count it; we add just the incremental UK & UAE cash (~₦79.8M for 26-Jun). Total Position then reconciles to Finance's GRAND TOTAL (₦2.078B) within ~0.01% (FX/rounding noise between the two sheets). UI: an **Available Liquidity (Group)** KPI card (available ₦ + encumbered-of-gross note) and a **Group Cash by Entity** table (per-entity Amount/Encumbered/Available + a GRAND TOTAL row showing Finance's exact figures), both rendered only when the sheet is present.
...
**Layout (top → bottom):** 5 KPI cards (Total Revenue, Gross Profit, EBITDA, **Net Profit**, **ARR**); two profitability gauge cards (net & gross margin vs target); **Monthly Performance Trend** line chart (see below); **Key Financial Ratios** grid (13 cards: gross/EBITDA/operating/net margin, annualised ROA & ROIC, OpEx-/payroll-/marketing-to-revenue, effective tax rate, interest coverage, current & cash ratio); **Economic Value Added (EVA)** card; Critical Insights; Income Statement; Expense Analysis + Revenue-by-Vertical chart; revenue breakdowns; Top Customers; Balance Sheet & Liquidity.

**"Profit After Tax" is displayed as "Net Profit"** everywhere user-facing (KPI card, income-statement row, profitability insight, ROA sub-label). The *parser* still matches the Summary tab's own `Profit After Tax` label (`get('profit after tax')`) — only the display nomenclature changed.

**Monthly Performance Trend chart:** `extract_monthly_trend()` reads the report's **"MoM"** tab — month-end date headers in **row 1**, line-item labels in **column C** — and plots **Revenue, Gross Profit, EBITDA, Net Profit** (NGN) as a Chart.js line chart. Parsing is label-driven (`total revenue` / `gross margin` [= gross profit naira] / `other income` / `total operating expenses` / `pat`) and date-driven: only columns whose row-1 header is a real `datetime` are read, so the `FY 25`/`FY 26` summary columns are skipped automatically. **EBITDA is DERIVED, not read from the MoM `EBITDA` row** — that row is corrupt in the source workbook (wrong sign/magnitude, e.g. Jan-26 shows −₦193.7M when true EBITDA is +₦188.5M, which made the chart plot a negative EBITDA above a positive Net Profit). It is computed with the **Summary tab's own formula** `EBITDA = Gross Profit + Other Income − Total Operating Expenses`, which reconciles exactly to each month's PAT. (The Summary YTD EBITDA covers one more month than the MoM tab, so the two won't sum-match — both are correct for their period.) It is **limited to the current fiscal year** (the latest year with revenue data — auto-advances each year) and drops months with no revenue. `main()` stashes the result on `metrics['trend']`; if the MoM tab is missing the chart is simply omitted (fails safe). The MoM figures track the consolidated group (Nigeria ≈ Group here, ~1% apart).

**ARR (KPI card):** mirrors the **Revenue & Fundability** dashboard's **"ARR As At"** card so both tabs show the *same* figure — the recurring share of revenue **actually EARNED YTD** measured against the **50% target** (NOT the planned/contracted ~$2.86M annual figure). The generator imports `generate_revenue_dashboard.extract_revenue_data()`, finds the Path to Revenue file in the same data folder, and computes `pct = Σ ytd_actual(recurring deals) / Σ ytd_actual(all deals) × 100` (the same formula as that dashboard's `arr_asat_pct`), with the month label from the last column with data. Card shows `<pct>%` (red when < 50%), `Target 50% · ▼ Npts below`, and `$X recurring of $Y earned YTD`. The report's own `ARR`/`ARR (%)` Summary lines are **only a fallback** if the Path to Revenue file is absent; the report-based "Recurring revenue contracted" YoY insight and the old "ARR % of revenue" ratio card were removed to avoid a conflicting number. **Note:** the value tracks the *live* Google-Sheet data on Streamlit Cloud (the local fallback xlsx may show a different %, e.g. 19% local vs 39% live) — by construction it equals whatever the Revenue & Fundability tab shows.
...
**Auto-latest selection:** Finance drops a new `Group Financial Report_<Mon-YY>.xlsx` into `data/` each week. `find_file()` returns the **latest** report, not `glob()[0]`: `_report_period_key()` parses a **month token + year** from anywhere in the filename (+ optional `_vN` version suffix) and ranks by (year, month, version, mtime). It anchors on a real month abbreviation (`jan…dec`), so naturally-named files like `Group Financial Report 18th Jun-26.xlsx`, `Group Financial Report_Jun-26.xlsx`, or `... Jun 2026.xlsx` all resolve to (2026, 6) — words such as "Report 18" are no longer mistaken for a period. Unparseable names fall back to mtime so they never outrank a dated report. The dashboard regenerates on each load / "Regenerate Dashboards" / 24h refresh, so a newer-dated file is picked up automatically — no hook/watcher needed. Period label and all figures come from inside the chosen workbook, so they update with the file.
...
- **USD Balance (incl. Investments)** = `usd_raw + investment_usd_raw`, displayed in actual dollars via `fmt_usd()` (NOT NGN-equivalent). **`usd_raw` is the CLOSING USD cash, DERIVED as `usd_closing_ngn / fx_rate`** (col J of the "TOTAL CASH (USD)" row ÷ the report's own FX). It is NOT read from col D of that row — col D is the **OPENING** balance (header "OPENING BALANCE", = prior week's closing) and was the source of a bug where the KPI lagged a week (fixed 2026-06-22). The derived value reconciles exactly to the report's own "CASH BALANCE" summary-block USD row. **`investment_usd_raw` is sourced from the authoritative "Cash Balance Summary" sheet** (`extract_cash_summary()`), NOT the working "Cash Report" tab: it sums the **AIF Investment Fund** + **Other Dollar Investment** blocks (fixed 2026-06-22 — the Cash Report tab's `TOTAL INVESTMENT (USD)` col H omitted the separate "Other Dollar Investment", e.g. the $41,652 FBNQuest/MinIo-Glo holding, and its AIF total disagreed with the summary; 19-Jun = $1,355,686.95). When the summary sheet is absent (older reports), it falls back to the Cash Report tab's col H. **NGN investment (`investment_ngn`) stays on the Cash Report tab** — the summary's "Other Naira" block is incomplete in older reports (omits the large NGN fixed deposits), so it is NOT overridden.

The "Cash Balance Summary" sheet layout: col B=label, C=Principal/Naira amount, D=Interest/USD amount, E=Total/Exchange rate, F=Naira Equivalent. Investment sections are headed `BREAKDOWN OF AIF INVESTMENT FUND` (USD), `BREAKDOWN OF OTHER DOLLAR INVESTMENT` (USD), `BREAKDOWN OF ... NAIRA INVESTMENTS` (NGN); each fund's current value is col E. `MARKET VALUE OF MTN SHARES` is read separately into `mtn_shares_ngn` (its own card — see below) and is **excluded from the cash position / Total Position** (listed equity, not cash-equivalent). `extract_cash_summary()` takes the latest "Market value as at …" row's rightmost numeric cell (the figure column shifts F↔G between report layouts). Older reports add a GBP column to the cash section (shifting Exchange rate/Naira Equivalent right), but the investment-section "Total" stays col E.

- **MTN Shares (Market Value)** = `mtn_shares_ngn` (NGN), rendered as its own KPI card right after USD Balance with a WoW change and the note "listed equity, excl. cash position". Card only renders when a value is present.

**Number format:** the three position balance cards (Total Position, NGN Balance, USD Balance) and the MTN card use **2-decimal abbreviated formatting** (`fmt_naira_precise()` / `fmt_usd_precise()` — same B/M/K suffix as `fmt_naira()`/`fmt_usd()` but 2 decimals across all magnitudes, e.g. ₦2.12B, ₦100.50M, $1.49M, ₦96.00M). Flow cards (Weekly Inflow/Outflow, Net Cash Flow, 4-Week Forecast) keep the compact `fmt_naira()`/`fmt_usd()` suffix format.
