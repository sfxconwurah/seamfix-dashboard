...
**ARR (KPI card):** sourced from the **Revenue & Fundability** dashboard so both tabs show the *same* figure — the generator imports `generate_revenue_dashboard.extract_revenue_data()`, finds the Path to Revenue file in the same data folder, and computes ARR = Σ annual USD (col F) of deals flagged "Recurring" (col D), with NGN = ARR × `FX_RATE` (1450). Currently ~$2.86M / 17 recurring streams. The report's own `ARR`/`ARR (%)` Summary lines are **only a fallback** if the Path to Revenue file is absent; the report-based "Recurring revenue contracted" YoY insight and the old "ARR % of revenue" ratio card were removed to avoid showing a conflicting ARR number.
